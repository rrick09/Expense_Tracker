...
def save_bank_statement():
    """
    Saves the bank statement and remaining budget to a text file as a dictionary.
    """
    try:
        with open(BANK_STATEMENT_FILE, "w") as file:
            json.dump({"transactions": bank_statement, "budget": budget}, file, indent=4)
    except OSError as e:
...
def load_bank_statement():
    """
    Loads the bank statement and remaining budget from a text file if it exists.
    """
    global bank_statement, budget
    try:
        with open(BANK_STATEMENT_FILE, "r") as file:
            data = json.load(file)
            bank_statement = data.get("transactions", {})
...
def money_left():
    """
    Calculates remaining budget after expenses and saves it.
    """
    global budget
    total_expenses = sum(exp['amount'] for exp in bank_statement.values() if exp['amount'] < 0)
    total_income = sum(exp['amount'] for exp in bank_statement.values() if exp['amount'] > 0)
    budget = total_income + total_expenses
    save_bank_statement()  # Save updated budget
...
def total_spent():
    ...
    Includes previously saved data from the bank statement.
    """
    load_bank_statement()  # Load saved transactions and budget
